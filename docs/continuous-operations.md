...
| **An execution boundary for the evidence lanes**, declared as `TRAUST_SANDBOXED_RUNNER=<runner-label>` | remediate-finding Phase 4b (mutation), property-test (property differential), run_checks.sh build/test leg | These lanes run the TARGET's own test code. They accept either nested podman or — on a runner that is already a credential-free, network-restricted job whose image carries the toolchain — this declaration. Without one of the two they emit `not_attempted` and the lane produces no evidence, silently costing you the strongest fix-side signal. It is a statement of deployment fact, not a security control, and the resolved boundary is recorded in every evidence item. Detection is `podman info`, so a job image that ships the podman CLI without a working runtime correctly falls through to the attested path rather than failing with exit 125. |
